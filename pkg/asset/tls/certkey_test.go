...
func TestCertKeyGenerate(t *testing.T) {
	...
	installConfigState, _ := installConfig.Generate(nil)
	...
	for i, tt := range tests {
		...
		keyFileName := filepath.Join(testDir, "tls", tt.certKey.KeyFileName)
		crtFileName := filepath.Join(testDir, "tls", tt.certKey.CertFileName)
